...
//------------Receive events------------//
  if(Serial.available() > 0) {
    ...
    // if the received inChar is digital
    if (isDigit(inChar)) {
    ...
    if (inChar == '\n') 
    {
      ...
      // judge x or y axis, down sample from 128*128 to 8*8
      if(X_axis)
      {
        x = Num/16;
      }
      ...
      {     
        ...
        if(1) //(current_event - previous_event)>15000
        {
          analogWrite(11, 500);
...
//--------------Send spikes-------------//  
  ...
  if(previous_level == current_level)
  {
    ...
    if(ind_Num==0)
    {
      if(1) //(current_packet-previous_packet)>15000
      {
        ...
        if(Num_com==10)
        {
          ...
          switch(ARMpos)
          {
            ...
            case 7:
              ...
              break; 
            default:
            //  Serial.println("Error: neurons more than 8");
              break;
          }
        }  
}
...
int decodeSeven(int current[7], int previous[7])
{
  ...
  switch(arr[0])
  {
    case 0:
      ...
      else {Serial.println("Error in 2-to-7 decode");}
      break;
    case 1:
